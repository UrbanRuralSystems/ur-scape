...
public class ReachabilityTool : Tool
{
    ...
    public EditSpeedPanel editSpeedPanelPrefab;
    public NDASetupPanel nDASetupPanelPrefab;
    public NDAParametersPanel nDAParametersPanelPrefab;
    ...
    public Toggle networkDisruptionAnalysisToggle;
    ...
    public readonly List<MobilityMode> mobilityModes = new List<MobilityMode>();
    ...
    private EditSpeedPanel editSpeedPanel;
    public NDAParametersPanel NDAParamPanel { private set; get; }
    ...
    protected override void Awake()
    {
        ...
        // Initialize classification speed panel
        editSpeedPanel = Instantiate(editSpeedPanelPrefab, centerTopContainerT, false);
        editSpeedPanel.gameObject.SetActive(false);

#if !UNITY_WEBGL
        NDAParamPanel = Instantiate(nDAParametersPanelPrefab, centerTopContainerT, false);
        NDAParamPanel.gameObject.SetActive(false);
#endif

        NDAAnalysisCompletedNotification = Instantiate(notificationPrefab, centerTopContainerT, false);
        NDAAnalysisCompletedNotification.gameObject.name = "NDAAnalysisCompletedNotification";
        NDAAnalysisCompletedNotification.gameObject.SetActive(false);

        NDASetupCompletedNotification = Instantiate(notificationPrefab, centerTopContainerT, false);
        NDASetupCompletedNotification.gameObject.name = "NDASetupCompletedNotification";
        NDASetupCompletedNotification.gameObject.SetActive(false);
    ...
    private void Update()
    {
        if (NetworkDisruptionAnalysis.isAnalysisCancelled)
            AnalysisCancelled();

        if (NetworkDisruptionAnalysis.isAnalysisCompleted)
            AnalysisCompleted();

        if (NetworkDisruptionAnalysis.isSetupCompleted)
            SetupCompleted();
    }
#endif
    ...
    private void OnLanguageChanged()
    {
        ...
        editSpeedPanel?.OnLanguageChanged();
    ...
    private void OnMobilityToggleChanged(int index, bool isOn)
    {
        if (isOn)
        {
            ...
            editSpeedPanel?.UpdateSpeeds(index);
        }
    }
    ...
    private void OnEditSpeedToggleChanged(bool isOn)
    {
        ...
        networkDisruptionAnalysisToggle.interactable = !isOn;
    }
    ...
    public void OnSpeedInputChanged(int index, string val)
    {
        var mode = mobilityModes[mobilityMode];
        if (!float.TryParse(val, out float speedVal))
            return;

        mode.speeds[index] = speedVal * kmPerHourToMetersPerMin; //convert from km/h to m/min
        mode.invSpeeds[index] = mode.speeds[index] > 0 ? 1f / mode.speeds[index] : 600;

        if (placeStart != null)
        {
            placeStart.SetMinutesPerMeter(mode.invSpeeds);
    ...
    private void ShowError(string msg)
    {
        ...
        networkDisruptionAnalysisToggle.isOn = false;
    }
...
#if !UNITY_WEBGL
    private void AnalysisCancelled()
    {
        networkDisruptionAnalysisToggle.isOn = false;
        
        NetworkDisruptionAnalysis.currActiveProcess = null;
        NetworkDisruptionAnalysis.isAnalysisCancelled = false;
        // Debug.Log("AnalysisCancelled");
    }

    private void AnalysisCompleted()
    {
        NDAAnalysisCompletedNotification.gameObject.SetActive(true);
        NDAParamPanel.runButton.gameObject.SetActive(true);
        NDAParamPanel.message.SetActive(false);
        NDAParamPanel.cancelButton.gameObject.SetActive(false);
        networkDisruptionAnalysisToggle.isOn = false;
        
        NetworkDisruptionAnalysis.currActiveProcess = null;
        NetworkDisruptionAnalysis.isAnalysisCompleted = false;
        // Debug.Log("AnalysisCompleted");
    }

    private void SetupCompleted()
    {
        NDASetupCompletedNotification.gameObject.SetActive(true);
        networkDisruptionAnalysisToggle.interactable = true;

        NetworkDisruptionAnalysis.isSetupCompleted = false;
        // Debug.Log("SetupCompleted");
    }
#endif
    ...
    public void LoadSiteMobilityModes()
    {
        ClearMobilityModes();
