...
public class LineInspectorPanel : MonoBehaviour
{
    ...
    private void OnLineInspectorToolLeftMouseDown()
    {
        ...
        Coordinate coords = mapController.GetCoordinatesFromUnits(worldPos.x, worldPos.z);
    ...
    public void AllowRemoveLineInspections(bool allow)
    {
        for (int i = 0; i < maxInspectionCount; ++i)
        {
            ...
            lineInfos[i].lineInspectionDelete.ShowInspectionDelete(allow);
            if (allow)
            {
    ...
    {
        inspectorOutput.LineOutput.ComputeAndUpdateTotalLength(lineInfos, lineInspector.CurrLineInspection);
		inspectorOutput.ComputeAndUpdateMetrics(InspectorTool.InspectorType.Line);
    }
    ...
    public void LineShowGridAndUpdateOutput(GridMapLayer mapLayer, bool show)
    {
        if (lineInspector.CurrLineInspection >= 0)
        {
			...
			inspectorOutput.ShowInspectorOutputItemLabel(InspectorTool.InspectorType.Line, dataLayerName, show);
			UpdateOutput();
        }
    }
    ...
    private void UpdateOutput()
	{
		...
		inspectorOutput.LineOutput.UpdateLineInspectorOutput(lineInfos[lineInspector.CurrLineInspection], dataLayers);
    }
