...
public class MidPt : ControlPt
{
	...
	protected override void UpdatePosOnDrag()
	{
		...
		LineInfo.controlPtsTB[0].transform.localPosition = LineInfo.mapViewAreaChanged ?
														   (
															lineInfo.scaleFactor.Equals(1.0f) ? newStart / canvas.scaleFactor :
																								newStart * lineInfo.scaleFactor
														   )
														   : newStart;
		LineInfo.controlPtsTB[1].transform.localPosition = LineInfo.mapViewAreaChanged ?
														   (
															lineInfo.scaleFactor.Equals(1.0f) ? newEnd / canvas.scaleFactor :
																								newEnd * lineInfo.scaleFactor
														   )
														   : newEnd;

		LineInfo.lineInspectionDelete.transform.localPosition = (newStart + newEnd) * 0.5f;
