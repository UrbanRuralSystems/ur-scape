...
public class ReferenceFinder : EditorWindow
{
	...
	void FindObjectReferences(Object toFind)
	{
		...
		if (m_paths == null)
		{
			...
			GetFilePaths("Assets", ".prefab", m_paths);
		}
		...
		for (int i = 0; i < numPaths; ++i)
		{
			...
			if (tmpArray[0] != toFind) // Don't add self
			{
	...
	public static void GetFilePaths(string startingDirectory, string extention, List<string> paths)
	{
		try
		{
			...
			{
				GetFilePaths(directories[i], extention, paths);
			}
		}
