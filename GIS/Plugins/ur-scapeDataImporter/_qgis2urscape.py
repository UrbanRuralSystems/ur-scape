...
---------------------------------------------------------------------"""
outputPath = "C:/Unity/ur-scape/Data" # Do not use any backslash (e.g. C:/Documents/ur-scape/Data)
name = "Floods (F2) FU Q20" # Set layer name in Camel case (e.g. Population Density)
...
"""Please set metadata"""
units = "meters" # set layer units (e.g. People per Sq km or set number from list: 0 = 'population/Sq2', 1 = AMSL )
location = "SW" # set location of layer (e.g. Palembang)
source = "FATHOM 2" # set source of layer (e.g. Statistics Bureau)
date = "2020" # Year of data in format YYYY.MM.DD (month and day are optional)
color = "1" # 0=Red, 1=Orange, 2=Yellow, 3=Light Green , 4 = Dark Green, 5 = Cyan, 6 = Light Blue, 7 = Dark Blue, 8 = Purple, 9 = Pink
group = "FATHOM 2 (rev. 90m)"
citation = "" # insert citation - 
mandatoryCitation = False # should the citation be always shown?
link = "" # link to dataset if aplied 
...
resolutionPatch = [1, 1, 5 , 10, 50, 360] # in degrees units
    ...
    def checkColors(self, inList):
        ...
        for layer in  inList:
            if any(name == s for s in layer):
                if (layer[2].isdigit()):
                    if not (int(layer[2]) == color[0] and int(layer[3]) == color[1] and int(layer[4]) == color[2]):
