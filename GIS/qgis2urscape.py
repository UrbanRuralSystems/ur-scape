...
---------------------------------------------------------------------"""
outputPath = "Insert Folder Path"  # Do not use any backslash (e.g. C:/Documents/ur-scape/Data)
...
resolution = "2" # 0-Neighbourhood, 1-City, 2-Metropolitan, 3-National, 4-Continental, 5-Global
...
resolutionLevels = [5, 100, 300, 0.0083333, 0.08333333, 0.416] 
...
def ProblemsInputCheck():
    ...
    try: # check field name input
        if hasattr(iface.activeLayer(), 'fields'):
            layer = layer = QgsVectorLayer(iface.activeLayer().source(),"polygon","ogr")
    ...
    try:
        res = res= resolutionLevels[int(resolution)] 
        epsg = 'EPSG:'+ resolutionEPSG[int(resolution)]
        layerTest = iface.activeLayer()
        if not hasattr(layerTest , 'fields'):
            rasterToDegress= RasterToUnits(layerTest ,res, epsg)
        else:
            rasterToDegress =VectorToRaster(layerTest ,layerTest, res,epsg, "test",False)
        rasterToGrid = MetresToDegress (rasterToDegress, "Test", res,epsg)
    except:
        print( "Opps, problem with projection, please check again")
        problem = True  
...
def RunAlgorithm():
    ...
    global maxPatchSize, totalCount #, year
...
def WriteLayerToCsv(nameLayer, groupName, layersCsvPath):
        ...
        with open(layersCsvPath, 'r',newline='',encoding='utf-16-le') as csvfile:
            ...
            for row in  spamreader:
                if len(row)>1: #in case of empty line, after manual edit of file
                    ...
                    if str(nameLayer) == str(row[1]):
                        ...
                        break
        if missingInFile:
            with open(layersCsvPath, 'a',newline='',encoding='utf-16-le') as csvfile:
...
def CreateCategoryVector(layer, type):
...
def VectorToRaster(layerIn, extent, setResolution, epsg, thisName,makeBigger):
    ...
    reprojectedRaster = QgsProcessingUtils.tempFolder()+"/Rasterized_Layer.tif"
...
def MetresToDegress (layerForWarp, thisName, res, epsg):   
    rasterDegress = QgsProcessingUtils.tempFolder()+"/Raster_Degress.tif"
...
def GetCleanExtent(gt,countX,countY):
    if extentAsCanvas:
        """if it is for municipal Budget then is extend as canvas"""
        """TODO refactor under Municipal budeget redevelopment"""
        my_crs=QgsCoordinateReferenceSystem(4326)
        QgsProject.instance().setCrs(my_crs)
        ...
        maxX=round(gt[0]+gt[1]*countX, ndigits)
        maxY=round(gt[3]+gt[5]*countY, ndigits)
...
def CreateGridFile(categories, gt, bandData,thisName, index, countX,countY,isPoint):
    ...
    minX,minY,maxX,maxY = GetCleanExtent(gt,countX,countY)
    ...
    ending = '_grid.csv'
    fileStringTemp = name+ sign +location+'@'+ str(index)+'_'+dateCode+ ending
    ...
    if citation.strip() and citation != "Insert Citation":
        if mandatoryCitation:
            output_file.write("MandatoryCitation," + citation + end)
        else:
            output_file.write("Citation," + citation + end)
    if link.strip() and link != "Insert Link":
        output_file.write("Link," + link + end)
    ...
    if not forMunicipalBudget:
        if not category and units.strip() and units != "Insert Units":
    ...
    for y in range(0, countY):
        for x in range(0, countX):
            ...
            value = 0 if float(value) < 0 else value
...
def ClipRaster (rasterIn, extent, index, cat ):
    ...
    if debuggingMode:
        ...
        QgsProject.instance().addMapLayer( layerTesting ) # adding to canvas
    """-
    finalRaster = QgsProcessingUtils.tempFolder() +"/FinalRaster.tif"
    ...
    if debuggingMode:
        layerTesting2 = QgsRasterLayer(finalRaster,"Final Clip Raster" + (str)(index))
        QgsProject.instance().addMapLayer( layerTesting2 ) # adding to canvas
    """
    return clipRaster 
...
def CheckLayerByType(activeLayer,res, epsg):
    ...
    if isinstance(activeLayer,QgsVectorLayer) :
        ...
        if layer.geometryType() == 0 : # Points Export
            ...
            category = activeLayer.fields().field(field).type()==10
            if category:
                categories, catLayer = CreateCategoryVector(layer,"LineString")
            
            layerIn = catLayer if category else layer

            """layerToRaster = FixGeometry(layerIn.source())"""
            rasterToDegress = VectorToRaster(layerIn,layer, res, epsg, name,False)
            ...
            category = activeLayer.fields().field(field).type()==10
            if category:
                categories, catLayer = CreateCategoryVector(layer,"Polygon")
...
def CheckLayerByScenario(activeLayer, resolution, epsg):
    ...
    if forReachability:
        ...
        if layer.geometryType() == 1:
            ...
            print ("Done here, have a nice day")
        else: 
        ...
        if layer.geometryType() == 2:
            ...
            layerIn = catLayer if category else layer    
            layerToWarp =VectorToRaster(layerIn,layer, resolution,epsg, name,False)
